...
import { useColorForm } from "./hooks";

interface ColorFormPageProps {
  colorId?: number;
}

export default function ColorFormPage({ colorId }: ColorFormPageProps) {
  const { formData, updateFormField, loading, saveLoading, error, saveError, handleSave } =
    useColorForm(colorId);

  if (loading) {
    return (
      <div className="flex justify-center py-16">
        <LoadingSpinner message="Cargando color..." />
      </div>
    );
  }
    ...
    <div className="card bg-base-100 shadow-lg">
      <div className="card-body">
        ...
        <div className="flex justify-end gap-2">
          <Link href="/apps/commerce/colors" className="btn btn-ghost">
            Cancelar
          </Link>
