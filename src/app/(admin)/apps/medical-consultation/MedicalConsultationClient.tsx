...
const ResultadoBusqueda = ({
  ...
  }, [activeIndex, filteredPeople, onSelectPerson]);
...
const ConsultaForm = forwardRef<ConsultaFormRef, ConsultaFormProps>(
  ({ medicalConsultation }, ref) => {
    ...
    const [
      medicalConsultationSubCategories,
      setMedicalConsultationSubCategories,
    ] = useState<IMedicalConsultationCategory[]>([]);
    ...
    const [formData, setFormData] = useState({
      ...
      insumo: "",
      objective: medicalConsultation?.objective ?? "",
      subjective: medicalConsultation?.subjective ?? "",
    });
    ...
    useEffect(() => {
      ...
      const fetchMedicalConsultationCategories = async () => {
        try {
          ...
          const subcategories =
            await medicalCategoryService.findAllSubCategories(
              schoolId,
              1,
              1000
            );
    ...
    useImperativeHandle(ref, () => {
      return {
        submitForm: () => {
          const cleaned: Record<string, string | null | number> = {
            ...formData,
          };
          ...
          return { ...cleaned, supply_ids: selectedInsumos.map((s) => s.id) };
        },
        focusFirst: () => {
    ...
    const handleChange = (
      ...
      const isNumber = () =>
        type === "number" || ["category_id", "sub_category_id"].includes(name);
    ...
    return (
      <div className="">
        <div className="grid grid-cols-12 gap-4 pb-4">
          ...
          <div className="col-span-12">
            <label className="fieldset-label">Objetivo</label>
            <input
              className="input input-xs mt-2 w-full"
              type="text"
              name="objective"
          ...
          <div className="col-span-12">
            <label className="fieldset-label">Exploración Física</label>
            <input
              className="input input-xs mt-2 w-full"
              type="text"
              name="subjective"
          ...
          <div className="col-span-12">
            ...
            >
              {[
                ...
                {
                  value: "JOCELYN ELIZABETH ESCAMILLA HUERTA",
                  text: "JOCELYN ELIZABETH ESCAMILLA HUERTA",
                },
                {
                  value: "HERNANDEZ RODRIGUEZ ITZEL",
                  text: "HERNANDEZ RODRIGUEZ ITZEL",
                },
              ].map((p) => (
...
export default function MedicalConsultationClient() {
  ...
  const handleSubmitConsulta = async () => {
    try {
      if (!consultaFormRef.current || !person || !schoolId) return;
    ...
    <main className="h-screen">
      ...
      {person === null ? (
        ...
        <>
          ...
          {showModalConsulta && (
            <dialog id="my_modal_5" className="modal modal-middle modal-open">
